...
interface SelectionFieldProps extends ControllerRenderProps {
  defaultValue?: string;
  placeholder?: string;
  options: Array<{
    value: string;
...
export const SelectionField = forwardRef<
  HTMLButtonElement,
  SelectionFieldProps
>(({ defaultValue, placeholder, options, onChange, ...rest }, ref) => (
  <Select.Root defaultValue={defaultValue} onValueChange={onChange} {...rest}>
    <Select.Trigger
      className="flex items-center justify-between gap-3 h-12 py-4 px-3 rounded border-zinc-800 border-2 bg-zinc-900 w-full hover:bg-zinc-800 hover:border-zinc-700 focus-within:!border-zinc-400 focus-within:bg-zinc-800 outline-none text-sm"
      ref={ref}
    >
