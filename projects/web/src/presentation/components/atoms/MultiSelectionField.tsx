...
>(({ value, children, removeItem, ...rest }, ref) => (
  <ComboboxItem
    value={value}
    focusOnHover
    className="py-3 px-3 [&:not(:last-child)]:border-b border-b-zinc-800 data-[active-item]:bg-zinc-800 outline-none text-sm flex flex-row justify-between items-center gap-2"
    ref={ref}
    {...rest}
  >
...
>(
  (
    { icon, data, createItem, onChange, defaultValue, removeItem, ...rest },
    ref,
  ) => {
    ...
    const handleKeyDown = async (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (e.key === 'Enter') {
        if (
          comboboxRef.current?.dataset.activeItem &&
    ...
    return (
      ...
      >
        <div
          className="flex items-center gap-3 min-h-12 py-2 px-3 rounded border-zinc-800 border-2 bg-zinc-900 w-full hover:bg-zinc-800 hover:border-zinc-700 focus-within:!border-zinc-400 focus-within:bg-zinc-800 cursor-text"
          onClick={() => {
